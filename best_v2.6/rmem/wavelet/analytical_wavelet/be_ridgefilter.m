...
if diff(OPTIONS.optional.TimeSegment([1 end]))*OPTIONS.automatic.sampling_rate+1 < OPTIONS.ridges.min_duration
    fprintf('In be_ridgefilter :\tRidges duration criterion is superior to data length. No ridges found\n')
    return
end
% -------------------------------------------------------------------------
...
fprintf('\n\n\n============ NEW PROCESS LAUNCHED : RIDGE-BASED FILTERING ============\n\n')
...
fprintf( '\nIn RidgeFilter :\t%i ridge lines extracted.\n', numel(R) )

% Save ridge-filtered signals
for ii = 1:numel(R)
    [OutputFiles{ii}, sMat{ii}] = keepRDG(R, C, ii, WData, OPTIONS);
end
fprintf('\nIn RidgeFilter :\tall ridge-based signals stored.\n');
...
%    db_reload_studies(OPTIONS.automatic.iStudy, 1);
    fprintf('\nIn RidgeFilter :\tnew signals added to database.\n');
end

% -------------------------------------------------------------------------
fprintf('\n============ RIDGE-BASED FILTERING : COMPLETE ============\n\n')
...
function [OutputF, sMat]    =   keepRDG(R, C, ii, WData, OPTIONS)
    ...
    if ~OPTIONS.automatic.stand_alone || OPTIONS.automatic.process
        ...
        Blims   =   bst_closest( OPTIONS.optional.BaselineSegment, OPTIONS.mandatory.DataTime ); 
        FUS.baseline    =  OPTIONS.mandatory.Data(idCH, Blims(1):Blims(2) ); 
    end
    % Adjust truncated TFtime
    dt          =   ( bst_closest( OPTIONS.automatic.TFtime(1),OPTIONS.mandatory.DataTime ) - 1 ) * (OPTIONS.wavelet.nb_levels+1);
    
    % Create a new signal from the extracted ridge lines
    sMat.F                  = NaN( OPTIONS.automatic.nb_channels, numel(R{ii}) );
    sMat.Fi                 = NaN( OPTIONS.automatic.nb_channels, numel(R{ii}) );
    sMat.Baseline           = NaN( OPTIONS.automatic.nb_channels, size(FUS.baseline,2) );
    ...
    if ~OPTIONS.automatic.stand_alone || OPTIONS.automatic.process
        ...
        OutputF         = fullfile(d1, [d2, '_RidgeFilter_', num2str(ii), d3]);
        
        % Save file
        FileName        = fullfile(OPTIONS.automatic.iProtocol.STUDIES, OutputF);
        bst_save(FileName, sMat, 'v6');
        % Add file to database structure
        db_add_data(idS, FileName, sMat);
...
function [OPTIONS]          =   get_selected_channels(OPTIONS)
    ...
    ChannelMat  = load( fullfile(OPTIONS.automatic.iProtocol.STUDIES, CH.FileName) , 'Channel');
...
if ~OPTIONS.automatic.stand_alone || OPTIONS.automatic.process
    ...
    while ii <= numel(DTfs) && ~stop
        tf      =   load( fullfile(iP.STUDIES, DTfs{ii}), 'Options', 'DataTypes', 'Time', 'DataFile' );
        ...
        if sum(C)==7
            TF  =   load( fullfile(iP.STUDIES, DTfs{ii}), 'TF', 'ridgeMAP', 'DataFile', 'Freqs' );
            W   =   TF.TF(RNG);
            S   =   TF.ridgeMAP(RNG);
