...
WData   = {};
scalo   = {};
newTime	= bst_closest( OPTIONS.optional.TimeSegment([1 end]),OPTIONS.mandatory.DataTime );
...
switch(ftype)
    ...
    case 'data'
        ...
        t = toc;
        fprintf('\tWavelet transform done in\t %f seconds\n', t);        
    ...
    case 'oversize'
        ...
        fprintf('\tTFplanes + scalograms computed done in\t %f seconds\n', t2)
...
if EST > 1000
    fprintf('\tover ''BEst'' limit. computing truncated planes\n')
    OPTIONS.automatic.TFcomment     =      [OPTIONS.automatic.TFcomment ' | truncated'];
    ftype   =   'oversize';
else
    ftype   =   'data';
    fprintf('\twithin ''BEst'' limit. computing full planes\n');
end
...
lim1    =   max(1,bst_closest(OPTIONS.wavelet.freqs_analyzed, OPTIONS.ridges.frequency_range(1) )-1);
lim2    =   min(nbF,bst_closest(OPTIONS.wavelet.freqs_analyzed, OPTIONS.ridges.frequency_range(2) ) + 1);
newTime	=   bst_closest( OPTIONS.optional.TimeSegment([1 end]),OPTIONS.mandatory.DataTime );
