function [WData, OPTIONS, data] = be_cwsparse(data, OPTIONS, varargin)
...
% --- limites spectrales de la decomposition
flims   =   OPTIONS.ridges.frequency_range;
aj2     =   f0/flims(1);
aj1     =   f0/flims(2);
J1      =   round( log2(aj1/a_min)/dj ) + 1;
J2      =   round( log2(aj2/a_min)/dj ) + 1;
frng    =   round(J1)-1:round(J2)+1;   
WData   =   sparse(J+1,N);
...
if n<=4
    brd = brd(n);
    brd = brd/(N-1)/dt;
    brd = bst_closest(OPTIONS.wavelet.freqs_analyzed, brd);
    WData(1:brd,:) = 0;
end
