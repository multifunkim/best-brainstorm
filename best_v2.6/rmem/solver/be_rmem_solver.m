...
for ii = 1 : nNEWdata
    ...
    % new filename
    [rep, base, ext] = bst_fileparts(OPTIONS2.optional.ResultFile);
    OPTIONS2.optional.ResultFile = fullfile(rep, [base, '_', num2str(ii), ext]);
    
    if OPTIONS2.automatic.stand_alone
    ...
    %else
    %    OPTIONS2.optional.DataFile   =   OPTIONS2.automatic.rMEMfiles{end};
    %    db_reload_studies( OPTIONS.automatic.iStudy );
    end
end
...
if ~OPTIONS.automatic.stand_alone
    ...
    Results = struct(...
        ...
        'ImagingKernel',    [], ...
        'MEMoptions',       OPTIONS); %, ...
        %'MEMdata',          rmfield(obj, 'ImageGridAmp') );
        
        if ii==nNEWdata
            db_reload_studies( OPTIONS2.automatic.iStudy );
        end
end
...
function [OPTIONS] = save_ridges_results(obj, OPTIONS, HM, ii)
% ===== SAVE RESULTS FILE =====
    ...
    Results.Time            = load( fullfile(OPTIONS.automatic.iProtocol.STUDIES, OPTIONS.automatic.rMEMfiles{ii}), 'Time' );
    ...
    Results.MEMoptions      = setfield(Results.MEMoptions, 'automatic', struct('entropy_drops', OPTIONS.automatic.entropy_drops,'final_alpha', OPTIONS.automatic.final_alpha, 'Comment', OPTIONS.automatic.Comment) );
    ...
    save( fullfile(iP.STUDIES, OPTIONS.optional.ResultFile), '-struct', 'Results');
...
%     % Expand data node
%     %panel_protocols('SelectNode', 'Results', iStudy, iResult, -1 );
%     
%     % If added to a 'default_study' node: need to update results links
%     if strcmpi(sStudy.Name, bst_get('DirDefaultStudy'))
...
function [OPTIONS, obj] = get_ridge_data(OPTIONS, obj, ii)
    ...
    elseif ~isempty(ii)
        ...
        if ischar(DT)
            DT              =   load( fullfile(iP.STUDIES, OPTIONS.automatic.rMEMfiles{ii}) );
            FRQs            =   load( fullfile(iP.STUDIES, DT.TFfile), 'Freqs', 'Options', 'TFpath', 'TFfile');
            FRQr            =   FRQs.Options.RidgesFRange;
            mspDATA         =   load( fullfile( iP.STUDIES, DT.History{end} ), 'F', 'Time');
        ...
        STd     = bst_closest( OPTIONS.optional.TimeSegment(1), DT.Time );
        NDd     = bst_closest( OPTIONS.optional.TimeSegment(end), DT.Time );
        OPTIONS.mandatory.Data              =   DT.F( :, STd:NDd );
        OPTIONS.optional.iData              =   DT.Fi( :, STd:NDd );  
        
        % Fill optional field : BASELINE
        OPTIONS.optional.Baseline           =   DT.Baseline(:, :);
        noise_covariance                    =   [];
        switch OPTIONS.solver.NoiseCov_method
            ...
            case {1, 2, 3}
                ...
                noise_covariance          	=   be_randomize_ridge_baseline(DT.Baseline, O);          
        end
        ...
        % Filter MSP DATA
        STm                 =   bst_closest( OPTIONS.optional.TimeSegment(1), mspDATA.Time );
        NDm                 =   bst_closest( OPTIONS.optional.TimeSegment(end), mspDATA.Time );
        %temp_sig            =   mspDATA.F(iDc, :);
        mspDATA.F(iDc,:)    =   be_bpfilter( mspDATA.F(iDc,:), OPTIONS.automatic.sampling_rate, FRQr );
