...
version = '2.6';
[StandAlone, process, install]  =   be_check_caller;
if ~StandAlone && ~install
    be_look_for_brainentropy;
end

% ===== common I/O arguments ===== %%

% Mandatory 
Def_OPTIONS.mandatory.version                   = version;
...
if ~stand_alone && isfield( OPTIONS, 'MEMpaneloptions' )
    ...
    MEMoptions.mandatory.Data                           =   zeros( numel(OPTIONS.Channel), numel(OPTIONS.DataTime) );
    MEMoptions.mandatory.Data(OPTIONS.GoodChannel,:)    =   OPTIONS.Data;
    % optional
    ...
    MEMoptions.automatic.DataInfo                       =   load( fullfile(MEMoptions.automatic.iProtocol.STUDIES, OPTIONS.DataFile) );
    % solver
    MEMoptions.solver.NoiseCov                          =   OPTIONS.NoiseCovRaw;
    % extra information
    Gain                        =   zeros( numel(OPTIONS.Channel), size(HeadModel.Gain,2) );
    Gain(OPTIONS.GoodChannel,:) =   HeadModel.Gain;
    HeadModel.Gain              =   Gain;
