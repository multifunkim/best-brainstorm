...
for ii = 1 : nMod
    ...
    OPTIONS.automatic.Modality(ii).channels   = intersect(OPTIONS.automatic.Modality(ii).channels,OPTIONS.automatic.GoodChannel);
    ...
    if ~isempty(OPTIONS.solver.NoiseCov)
        ...
        for i_sc = 1: NbjNoiseCov
            OPTIONS.automatic.Modality(ii).covariance(CH,CH,i_sc) = ...
