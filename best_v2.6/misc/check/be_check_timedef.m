...
OPTIONS.optional.TimeSegment        =   bst_closest( OPTIONS.optional.TimeSegment([1 end]), OPTIONS.mandatory.DataTime );
...
if isempty(OPTIONS.optional.Baseline) && ~any(isRF)
    ...
    if isempty(OPTIONS.optional.BaselineSegment)
        ...
        STb     = bst_closest( OPTIONS.optional.BaselineSegment(1), OPTIONS.optional.BaselineTime );
        NDb     = bst_closest( OPTIONS.optional.BaselineSegment(end), OPTIONS.optional.BaselineTime );
...
if strcmp(OPTIONS.mandatory.pipeline, 'cMEM') && strcmp(OPTIONS.clustering.clusters_type, 'static')
    ...
    tmSMP   =   bst_closest( OPTIONS.optional.TimeSegment([1 end]), OPTIONS.mandatory.DataTime );
    nSMP    =   diff( tmSMP ) + 1;
...
if ~isempty( OPTIONS.optional.EmptyRoom_data )
    ...
    MNch    =   OPTIONS.mandatory.ChannelTypes;
    ...
    elseif ~isempty(ERch)
        ...
        for ii  =   1 : numel(iMod)
            idE     =   find(strcmp(OPTIONS.optional.EmptyRoom_channels, ERch{iMod(ii)}));
            nFound  =   nFound + 1; 
            nERD( iMod(ii),: )  =   OPTIONS.optional.EmptyRoom_data(idE,:);
        end
