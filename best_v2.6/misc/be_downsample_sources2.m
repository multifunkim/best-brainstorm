...
cortex      =   load( fullfile(iP.SUBJECTS,FILE.SurfaceFile), 'Faces', 'Vertices', 'VertConn');
...
nanidx      =   ~any(isnan(FILE.ImageGridAmp));
src_ampl    =   FILE.ImageGridAmp(:, ~any(isnan(FILE.ImageGridAmp)) );
...
nIx =   single( 1:nbS );
long_in_short   =   single(1:nbS);
short_in_long   =   single(1:nbS);
for ii = 1 : nbS-1
    
    [maxima,imaxima]=   max(neigh1_val,[],2);
    ...
    if ~sum(maxima)
        ...
        neigh1_idx( idnnul,: )    	=   0;
        neigh1_val( idnnul,: )     	=   0;
        neigh1_idx( idnnul(1),1 )   = short_in_long( idnnul(2) );
        neigh1_idx( idnnul(2),1)    = short_in_long( idnnul(1) );
    ...
    id_max2             =   long_in_short( neigh1_idx(id_max1,id_max2_rel) );
    ...
    % - unique cluster
    neigh1_idx( id_max1, : )                =   0;
    neigh1_val( id_max1, : )                =   0;
    neigh1_idx( id_max1, 1:numel(neis) )    =   short_in_long(neis);
    ...
    if neis
        ...
        if r3
            ...
            neigh1_val( (c3-1)*(size(neigh1_idx,1)) + neis( r3 ) ) = dtable(3,r3);
        end
...
%         neigh1_val( neis, : ) = mindtable';
    ...
    neigh1_val( id_max2, : )                =   [];
    neigh1_idx( id_max2, : )                =   [];
    nidmax2                                 =   short_in_long(id_max2);
    long_in_short( nidmax2 )                =   0;
    long_in_short(nidmax2+1:end)            =   long_in_short(nidmax2+1:end)-1;
    long_in_short(long_in_short<0)          =   0;
    short_in_long(id_max2)                  =   [];
