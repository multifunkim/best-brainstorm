...
if isfield(vizData, 'hpar')
    ...
    TF = load( fullfile(iP.STUDIES, TFfile), 'Time', 'Freqs', 'Options', 'DataTypes' );  %'ridgeMAP', 
    TF.TFfile = TFfile;
    load( fullfile(iP.STUDIES, TF.TFfile), 'DataFile' )
    load(fullfile(iP.STUDIES, DataFile), 'Time')
    TF.rdgTIME = Time;
    ...
    LIMs    =   bst_closest(TF.Freqs, 1);
    RMP = load( fullfile(iP.STUDIES, TF.iD{1}), 'rdgMOD');
    RMP = RMP.rdgMOD{1};
    RMP(1:LIMs,:) = 0;
    TF.RMP = RMP;
    vizData.TF = TF;
        ...
        case 'Flims'
            try
                ...
                if isnumeric(NMarg) && numel(NMarg)==2
                    Flims = bst_closest(NMarg, TF.Freqs);
                end
            end
        case 'cx'
            cx = 1;
...
% Show cortical map
iS	= bst_get('Study', fullfile(bst_fileparts(TF.DataFile),'brainstormstudy.mat') );
if cx == 1 && ~isempty( find_RSLT(iD, iS, iP, rdgLIMS) )
    ...
    if ~isfield(vizData, 'CX')
        vizData.CX = load( fullfile(iP.SUBJECTS, TF.CXfile.FileName) );
    end
end
...
function [TF] = find_TF(iP)
    ...
    TP  =   strfind( fullfile(iP.STUDIES, FL), 'timefreq' );
    if numel(TP)==1
        O = load( fullfile(iP.STUDIES, FL), 'Options' );
    ...
    TP  =   whos( '-file', fullfile(iP.STUDIES, FL), 'Fi' );
    if numel(TP)==1 
        O = load( fullfile(iP.STUDIES, FL), 'TFfile', 'DataType' );
        if exist( fullfile(iP.STUDIES, O.TFfile), 'file' ) && any(strcmpi(O.DataType, {'data', 'recordings'}))
            TF = O.TFfile;
        else
    ...
    TP  =   whos( '-file', fullfile(iP.STUDIES, FL), 'DataType', 'F');
    if numel(TP)==2 && isempty(TF)
        iS       = bst_get('Study', fullfile(bst_fileparts(FL),'brainstormstudy.mat') );
        for ii = 1 : numel(iS.Timefreq)
            O = load( fullfile(iP.STUDIES, iS.Timefreq(ii).FileName), 'Options', 'DataFile' );
...
function [iD] = find_RDG(iP, TFfile)
    iS  =   bst_get( 'Study', fullfile( bst_fileparts(TFfile), 'brainstormstudy.mat' ) );
    iSd =   { iS.Data(:).FileName };
    iD  =   cell(0,2);
    
    warning('OFF')
    for ii = 1 : numel(iSd)
        a = load( fullfile(iP.STUDIES, iSd{ii}), 'TFfile' );
...
function [int] = find_RSLT(iD, iS, iP, lm)
    ...
    for ii =1 : size(iD,1)
        ...
        iii = load( fullfile( iP.STUDIES,rsF{idx(idx2(1))} ), 'ImageGridAmp' );
...
for ii = 1 : size(iD,1)
    ...
    load( fullfile(iP.STUDIES, iD{ii,1}), 'TFpath' );
    x = fix(TFpath/nbF) + 1;
    y = mod(TFpath, nbF);
    if ~isempty(lm)
