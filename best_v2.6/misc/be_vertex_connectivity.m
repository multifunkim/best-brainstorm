...
if ~OPTIONS.automatic.stand_alone
    load( fullfile(OPTIONS.automatic.iProtocol.STUDIES, OPTIONS.optional.DataFile), 'Time')
    ...
    if ~isempty(HeadModel.SurfaceFile) && exist(fullfile(protoc.SUBJECTS,HeadModel.SurfaceFile), 'file')
        VCfile      =   fullfile(protoc.SUBJECTS,HeadModel.SurfaceFile); 
        ...
        VCfile      = fullfile(protoc.SUBJECTS, cxfile.FileName);
        load( VCfile, 'VertConn', 'Vertices', 'Faces');
    end
