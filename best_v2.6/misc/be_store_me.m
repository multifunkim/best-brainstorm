...
if ~isempty(S.Channel)
    ...
    Ch  =   load( fullfile(Prot.STUDIES, S.Channel.FileName), 'Channel' );
...
if ~isempty(BSsub)
    surfF   =   load( fullfile(Prot.SUBJECTS, BSsub.Surface(BSsub.iCortex).FileName ) );    
...
AAT     =   load(fullfile(iProt.SUBJECTS,SUB.Surface(SUB.iCortex).FileName),'Atlas');

% Parent infos
if ~isempty( parent )
    INF =   load( fullfile(iProt.STUDIES, parent), 'Time' );   
else
...
Prot    =   bst_get('ProtocolInfo');
SUB     =   bst_get('Subject', S.BrainStormSubject);
AAT     =   load(fullfile(Prot.SUBJECTS,SUB.Surface(SUB.iCortex).FileName),'Atlas');
