...
function [bstPanelNew, panelName] = CreatePanel(OPTIONS,varargin)  %#ok<DEFNU> 
    ...
    % Check if WaveLab is in path/folder
    % be_look_for_wavelab;
    
    % Check caller
    caller      =   dbstack(1); 
    firstCall   =   0;
    if ~isempty(strfind( caller(5).name, 'ComputeInverse' )) 
        clear global MEMglobal        
        firstCall   =   1;
    elseif strcmp(caller(6).name, '@(h,ev)EditProperties_Callback(iProcess,optNames{iOpt})')
        clear global MEMglobal            
        firstCall   =   1;
    ...
    %% --------------------- REGULAR OPTIONS PANEL --------------------- %%
        ...
        % Version
        jTXTver =   JTextField(OPTIONS.mandatory.version);
        ...
        if isfield(OPTIONS.mandatory, 'pipeline') && any( strcmp(OPTIONS.mandatory.pipeline, {'cMEM','wMEM','rMEM'}) )
            ...
            JPanelparam.add('br', JLabel('Neuroimage 29 (3), 734-753'));
            JPanelparam.add('br', JLabel(' '));
            ...
            JPanelparam.add('br', JLabel('PLos One'));
            JPanelparam.add('br', JLabel(' '));
            ...
            JPanelparam.add('br', JLabel('IEEE TBME'));
            JPanelparam.add('br', JLabel(' '));
            ...
            JPanelparam.add('br', JLabel('Zerouali, Herry, Jemel and Lina (2013)'));
            JPanelparam.add('br', JLabel('IEEE TBME'));
            JPanelparam.add('br', JLabel(' '));
...
%% ===== OK BUTTON =====
    ...
    function SwitchPipeline(varargin)
        ...
        % Get new options
        MEMoptions          =   struct('mandatory', struct('pipeline', OPTIONS.mandatory.pipeline, 'version', OPTIONS.mandatory.version), 'automatic', struct('stand_alone', 1) ); 
        if any( strcmp(OPTIONS.mandatory.pipeline, {'wMEM', 'rMEM'}) )
        ...
        bstPanelNew = panel_brainentropy('CreatePanel', OPTIONS);
...
function s = GetPanelContents(varargin) %#ok<DEFNU>
    ...
    MEMpaneloptions.mandatory.version   =   char( ctrl.jTXTver.getText() ); 
...
if St > Nd
    ...
    if isnan(ST), ST = Time(1); else ST = Time( bst_closest(ST,Time) ); end
    if isnan(ND), ND = Time(end); else ND = Time( bst_closest(ND,Time) ); end
...
[Lst, Frmt]   = java_getfile( 'open', ...
        ...
        {{'.*'},                 'MEG/EEG: 4D-Neuroimaging/BTi (*.*)',   '4D'; ...
         ...
         {'.avr','.mux','.mul'}, 'EEG: BESA exports (*.avr;*.mul)',      'EEG-BESA'; ...
